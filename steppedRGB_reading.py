...
def write_file(file,rate):
    t = 1000/rate # ms
    for i in range(5 * rate): # Running in five seconds
...
if __name__ == "__main__":
    rate = 5 # 5 samples per second
    with open('./log.txt', 'w') as file:
        try:
            #start = time.ticks_ms()
            set_all_color([63, 0, 0]) # Red 25%
            write_file(file, rate)
            #end = time.ticks_ms()
            #print("Time: ", time.ticks_diff(end, start)) # Time monitoring
            set_all_color([127, 0, 0]) # Red 50%
            write_file(file, rate)
            set_all_color([191, 0, 0]) # Red 75%
            write_file(file, rate)
            set_all_color([255, 0, 0]) # Red 100%
            write_file(file, rate)
            set_all_color([0, 63, 0]) # Green 25%
            write_file(file, rate)
            set_all_color([0, 127, 0]) # Green 50%
            write_file(file, rate)
            set_all_color([0, 191, 0]) # Green 75%
            write_file(file, rate)
            set_all_color([0, 255, 0]) # Green 100%
            write_file(file, rate)
            set_all_color([0, 0, 63]) # Blue 25%
            write_file(file, rate)
            set_all_color([0, 0, 127]) # Blue 50%
            write_file(file, rate)
            set_all_color([0, 0, 191]) # Blue 75%
            write_file(file, rate)
            set_all_color([0, 0, 255]) # Blue 100%
            write_file(file, rate)
        finally:
            set_off()
